...
with open("../regional/areas_mapping.json", "r", encoding="utf-8") as f:
...
with open("vaccinations_data_history.json", "w", encoding="utf-8") as file:
    data.drop(columns=["areaid"]).to_json(
        file, orient="records", indent=3, force_ascii=False
    )
...
with open("cumulative_per_area_vaccinations.json", "w", encoding="utf-8") as file:
    cum_per_area_data.drop(columns=["areaid"]).to_json(
        file, orient="records", indent=3, force_ascii=False
    )
